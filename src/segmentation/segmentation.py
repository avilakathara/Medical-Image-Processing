...
if use_lines:
    ...
    for point in lines[0][2:-3].split("], ["):
        mandible.append(list(map(int,point.split(", "))))
...
for point in mandible:
    ...
    # test[flood(img,(point[0],point[1],point[2]),tolerance=5)] = 3
...
# prob = random_walker(img, test, beta=0.1, mode='cg_j',tol=0.1, copy=False, return_full_prob=True)
labels = random_walker(img, seeds, beta=0.1, mode='cg_j',tol=0.1, copy=False).astype(int)
print(time.time() - start_time, "seconds")

# labels[labels==1] = 0

# labels = np.zeros(structures.shape).astype(int)
# labels[prob[1]>=0.5] = 1
