...
def get_optimal_slice(uncertainty):
    ...
    for i in range(0, iterations):
        ...
        chosen_point = np.array([int(np.random.uniform(0, x)), int(np.random.uniform(0, y)), int(np.random.uniform(0, z))])
        #chosen_point = [10,10,10]
        # Generate a normal
        ...
        current_point, current_normal = gradient_descent(uncertainty, chosen_point, chosen_normal, point_step_size, normal_step_size, gradients)
...
def gradient_descent(uncertainty, start_pos, start_normal, point_step_size, normal_step_size, gradients):
    ...
    for i in range(40):
        ...
        # TODO: the normal update doesnt work yet
        #current_normal -= normal_step_size * np.array([update_value[0], update_value[1], 0])
...
def get_normal_update(normal, gradients, indexes, uv_indexes, point):
    ...
    #print("the sum of gradients is {}".format(sum2))
...
if __name__ == "__main__":
    ...
    test_arr = np.load('test_data/70sphere.npy')
